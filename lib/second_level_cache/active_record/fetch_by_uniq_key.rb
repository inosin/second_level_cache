  ...
  module ActiveRecord
    module FetchByUniqKey
      def fetch_by_uniq_key(value, uniq_key_name)
        ...
        if _id = SecondLevelCache.cache_store.get(cache_uniq_key(value, uniq_key_name))
          self.find(_id) rescue nil
        else
          record = self.where(uniq_key_name => value).first
          record.tap{|record| SecondLevelCache.cache_store.set(cache_uniq_key(value, uniq_key_name), record.id)} if record
        end
      end
